...
export const IdeaGeneratorFlow = () => {
  ...
  const generateIdeas = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-business-ideas', {
        body: {
  ...
  const deleteIdea = async (ideaId: string) => {
    try {
      await supabase.from('business_ideas').delete().eq('id', ideaId).eq('user_id', user?.id);
  ...
  if (step === 4) {
    return <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="grid gap-6">
          {generatedIdeas.map((idea, index) => <Card key={index} className="p-6 border-0 shadow-lg">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-bold text-gray-900">{idea.title}</h3>
                <Button variant="ghost" size="sm" onClick={() => deleteIdea(idea.id)} className="text-red-500 hover:text-red-700">
  ...
  return <div className="max-w-2xl mx-auto p-6">
      ...
      <Card className="p-8 border-0 shadow-lg">
        {step === 1 && <div className="space-y-6">
            ...
            <Textarea value={formData.skills} onChange={e => setFormData(prev => ({
          ...prev,
          skills: e.target.value
        }))} placeholder="e.g., Software development, digital marketing, finance, design, sales, project management..." className="min-h-32 font-inter " />
          </div>}
        ...
        {step === 2 && <div className="space-y-6">
            ...
            <Textarea value={formData.problems} onChange={e => setFormData(prev => ({
          ...prev,
          problems: e.target.value
        }))} placeholder="e.g., Finding reliable contractors, managing remote teams, tracking personal finances, booking appointments..." className="min-h-32 font-medium" />
          </div>}
        ...
        {step === 3 && <div className="space-y-6">
            ...
            <Textarea value={formData.industries} onChange={e => setFormData(prev => ({
          ...prev,
          industries: e.target.value
        }))} placeholder="e.g., HealthTech, FinTech, EdTech, SaaS, E-commerce, Sustainability, AI/ML..." className="min-h-32 font-medium" />
          </div>}

        <div className="flex justify-between mt-8">
          <Button variant="outline" onClick={() => setStep(Math.max(1, step - 1))} disabled={step === 1} className="font-medium">
            Previous
          </Button>
          <Button onClick={handleNext} disabled={isLoading || step === 1 && !formData.skills.trim() || step === 2 && !formData.problems.trim()} className="bg-blue-600 hover:bg-blue-700 font-medium">
