...
export const DashboardOverview = ({ onToolStart }: DashboardOverviewProps) => {
  const { user } = useAuth();
            ...
            return (
              <div key={tool.id} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-sm transition-shadow">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => onToolStart(tool.id)}
                  className="w-full justify-between text-xs text-gray-600 hover:text-gray-900"
                >
                  Open
